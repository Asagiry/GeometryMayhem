class_name BossPhase4State
extends BossState

const STAGE_INDEX: int = 3

static var state_name = "BossPhase4State"

func enter() -> void:
	_connect_or_disconnect_signals(true)
	_setup_timer()
	await boss.get_tree().create_timer(0.5).timeout
	attack_controller.prepare_stage(STAGE_INDEX, [0, 1, 2, 6])
...
func _on_stage_ready():
	...
	_parallel_execute_attack(attack_controller.current_stage_attacks[0])
	...
	_parallel_execute_attack(attack_controller.current_stage_attacks[6])
...
func _on_phase_changed():
	state_machine.transition(BossPhase5State.state_name)
